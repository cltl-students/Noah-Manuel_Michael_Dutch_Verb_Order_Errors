...
def shuffle_only_verb_positions():
    ...
    for split in ['test', 'dev', 'train']:
        ...
        for sent in df['spaced']:
            ...
            if [el for el in masked_sentence if el != 0] and len(masked_sentence) > 1:  # include sentence only if
                # parser was able to detect a verb and the sentence consists of more than 1 verb + 1 punc
        ...
        df_new.to_csv(f'{split}_shuffled_random_all_and_verbs.tsv', sep='\t', header=True, index=True,
                      encoding='utf-8')
