...
def preprocess_readability_corpus():
    ...
    df = pd.read_csv('../Unpermuted_Datasets/Originals/edia_readability_corpus.csv', sep=',', header=0,
                     encoding='utf-8')
    ...
    for i, row in df_texts.iterrows():
        ...
        for sent in doc.sents:
            if len(sent.text) > 10 and re.match(r'^[A-Z].*[.!?]$', sent.text):  # only sents that start with capital
                ...
                list_of_all_sentences.append(re.sub(r'[\t ]', ' ', sent.text))
    ...
    df_sents.to_csv('../Unpermuted_Datasets/readability_data.tsv', sep='\t', header=True, encoding='utf-8',
                    index_label='index')
