...
def check_if_verb_in_sent(spacy_pipeline, sents):
    ...
    for sent in sents:
        verb_in_sent = False
        doc = nlp(sent)
        if len(doc) <= 50:  # reduce length to 50 in line with length distribution in actual learner data
            for token in doc:
...
def get_datasplit():
    ...
    :return:
    """
    df_edia = pd.read_csv('../Unpermuted_Datasets/Originals/readability_data.tsv', sep='\t', encoding='utf-8', header=0)
    df_lassy = pd.read_csv('../Unpermuted_Datasets/Originals/lassy_data.tsv', sep='\t', encoding='utf-8', header=0)
    df_wainot = pd.read_csv('../Unpermuted_Datasets/Originals/wainot_data.tsv', sep='\t', encoding='utf-8', header=0)
    df_leipzig = pd.read_csv('../Unpermuted_Datasets/Originals/leipzig_data.tsv', sep='\t', encoding='utf-8', header=0)
