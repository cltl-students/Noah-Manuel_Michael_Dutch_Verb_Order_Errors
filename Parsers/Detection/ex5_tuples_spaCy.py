...
def run_experiment_5(datasplit):
    ...
    :return:
    """
    train_spacy_Correct = read_in_json_data_and_convert_to_str_sequence('Tuples/Spacy/tuple_data_spacy', 'train', 'Correct',
                                                                        spacy=True)
    train_spacy_split = read_in_json_data_and_convert_to_str_sequence('Tuples/Spacy/tuple_data_spacy', 'train',
                                                                      datasplit, spacy=True)

    # test_spacy_Correct = read_in_json_data_and_convert_to_str_sequence('Tuples/Spacy/tuple_data_spacy', 'test', 'Correct',
    #                                                                    spacy=True)
    # test_spacy_split = read_in_json_data_and_convert_to_str_sequence('Tuples/Spacy/tuple_data_spacy', 'test', datasplit,
    #                                                                  spacy=True)

    train_spacy_full = train_spacy_Correct + train_spacy_split
    # test_spacy_full = test_spacy_Correct + test_spacy_split
    ...
    gold_train_spacy_full = ['correct' for _ in range(len(train_spacy_Correct))] + \
...
if __name__ == '__main__':
    run_experiment_5('Rand')
    run_experiment_5('Verbs')
